...
const Seeuserdata = ({userDivdata,userDiv,setuserDiv}) => {
  return (
    ...
    <div className={`${userDiv} top-0 left-0 h-screen w-full flex items-center justify-center`}>
      <div className="bg-white rounded p-4 w-[80%] md:w-[50%] lg:w-[40%] text-zinc-500">
        ...
        <div className="mt-2">
          <label htmlFor="">Email:<span className='font-semibold'>
            {userDivdata.email}</span>
          </label>
        </div>
        <div className="mt-2">
          <label htmlFor="">Address:{" "}<span className='font-semibold'>
            {userDivdata.address}</span>
          </label>
        </div>
      </div>
    </div>
