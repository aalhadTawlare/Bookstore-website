...
const Cart = () => {
  ...
  useEffect(() => {
    const fetch = async()=>{
      const response = await axios.get("http://localhost:3000/api/v1/getbook-cart",{headers});
      setcart(response.data.data);
    }
    fetch();
  }, [cart])

  const delitmes = async(bookid) =>{
    const response = await axios.put(`http://localhost:3000/api/v1/removebook-cart/${bookid}`,{},{headers});
    ...
    {cart && cart.length === 0 && (
      <div className="h-screen">
        <div className="h-[100%] flex items-center justify-center flex-col">
          ...
          <img src="./Empty_cart.png" alt="" className='lg:h-[50vh]'/>
        </div>
      </div>
    )}
    {cart && cart.length > 0 && (
